...
<!-- Bootstrap 5 CSS -->
<script
	src="https://dapi.kakao.com/v2/maps/sdk.js?appkey=480ae76903e53687e5cad91c46fb2808&libraries=services"></script>
...
<link
	...
	rel="stylesheet">
<link rel="stylesheet"
	href="https://fonts.nununu.co.kr/gmarket-sans.css" />
</head>
...
<body>
	...
	<div class="main_container">
		...
		<div class="left_aside">
			...
			<input class="reset-btn" type="button" onclick="resetBtnAction()"
				value="필터 초기화">
				...
				<form action="/subMain" method="GET">
						...
						<c:forEach var="areas" items="${areasList}">
							<div class="filter-item">
								<div class="filterBtn-container">
									<button
										class="filterBtn ${areas.id == param.areasId ? 'selected' : ''}"
										type="button"
										onclick="filterAction(this, ${areas.id}, 'areas')"></button>
								</div>
								<div>${areas.name}</div>
							</div>
						</c:forEach>
					...
					<c:forEach var="categories" items="${categoriesList}">
						<div class="filter-item">
							<div class="filterBtn-container">
								<button
									class="filterBtn ${categories.id == param.categoriesId ? 'selected' : ''}"
									type="button"
									onclick="filterAction(this, ${categories.id}, 'categories')"></button>
							</div>
		...
		<div class="places_container">
			...
			<c:forEach var="places" items="${placesList}">
				<div class="places_box" data-keyword="${places.id}"
			...
			<div class="container mt-5">
				...
				<nav>
					<ul
						class="pagination pagination-sm justify-content-center rounded-pill">
						<!-- 이전 페이지 -->
						<li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
							<a class="page-link rounded-pill"
							href="?page=${currentPage - 1}&pageSize=5
