...
public class DetailDAOImpl implements DetailDAO {
    ...
    public List<Places> getPlaceDetail(int id) {
        ...
        return sqlSession.selectOne("detailMapper.getPlaceDetail", id);
    }
    ...
    public List<Places> getPlaceDetailByName(String placeName) {
        // 여러 개의 정보가 아닌 하나의 상세 정보를 조회할 때는 selectOne을 사용
        return sqlSession.selectOne("detailMapper.getPlaceDetailByName", placeName);
    }

    // 관광지 이미지 목록 조회
    @Override
    public List<String> getPlaceImages(int id) {
        return sqlSession.selectList("detailMapper.getPlaceImages", id);
    }
}
